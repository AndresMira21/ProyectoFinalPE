...
public class NLPAnalysisService {
    ...
    public AnalysisDatos.AnalysisResult analyze(String text) {
        ...
        // Clasificacion
        String type = TYPE_PATTERNS.entrySet().stream()
                .filter(e -> e.getValue().matcher(norm).find())
                .map(Map.Entry::getKey)
        ...
        AnalysisDatos.AnalysisResult res = new AnalysisDatos.AnalysisResult();
        res.setType(type);
    ...
    public List<String> answerQuestions(String text, String question) {
        String norm = normalize(text);
        // simple por búsqueda de palabras clave
